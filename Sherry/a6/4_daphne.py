...
for i in range(len(n_particles)):
    ...
    if i == 0:
        plot_histogram_evidence(samples[i].numpy(), mean, torch.exp(logZ),
                       "Histogram for mu when n-particles = {} in 4.daphne".format(n_particles[i]),
                       "mu", "4_daphne_{}".format(n_particles[i]))
    else:
        variance = np.var(samples[i].numpy())
        plot_histogram_variance_evidence(samples[i].numpy(), mean, variance, torch.exp(logZ),
                                         "Histogram for mu when n-particles = {}".format(n_particles[i]),
                                "mu", "4_daphne_{}".format(n_particles[i]))

mean_samples = torch.stack(mean_samples)
logZs = torch.stack(logZs)

plot_trace(mean_samples.numpy(), "Posterior expectation for 4.daphne", "mu", "4_daphne_trace_plot")
Zs = torch.exp(torch.FloatTensor(logZs))
plot_trace(Zs.numpy(), "Marginal probability/evidence estimate for 4.daphne", "probability", "4_daphne_evidence")