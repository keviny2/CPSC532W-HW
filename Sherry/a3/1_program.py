...
plot_histogram_IS(samples_IS.numpy(), weights.numpy(), "Posterior distribution of mu in 1.daphne using IS",
               "mu", "IS", "posterior_histogram_1_daphne")


'''Gibbs'''
t = time.time()
sample, variables_dict_set = gibbs(latent, variables_dict, 50000, edges, links, returnings)
...
logPs = joint_log_likelihood(joint_latent_set, links, variables_dict_set, 50000)
...
t = time.time()
samples, variables_dict_set = HMC(latents_dict, 50000, 10, 0.1, torch.eye(len(latent)), observes_dict, links)
...
joint_latent_set = latent.copy()
joint_latent_set.append(returnings)
logPs = joint_log_likelihood_HMC(joint_latent_set, links, variables_dict_set, 50000)
