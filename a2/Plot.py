...
for i in range(1, 18):
    ...
    ax.set(ylabel='Frequency', xlabel='Samplings from the prior distribution')
    ax.set_title("Histogram for state %s in 3.daphne from Evaluation-based Sampling" %i)
...
for i in range(1, 3):
    ...
    ax.set(ylabel='Frequency', xlabel='Samplings from the prior distribution')
    ax.set_title("Histogram for %s_0 in 4.daphne from Evaluation-based Sampling" % name[i - 1])
...
for i in range(3, 5):
    ...
    ax.hist(evaluation_4[:, i - 1])
    ax.set(ylabel='Frequency', xlabel='Samplings from the prior distribution')
    ax.set_title("Histogram for %s_1 in 4.daphne from Evaluation-based Sampling" %name[i - 1])
    fname = os.path.join("figs", "evaluation_4_%s" %i)
    plt.savefig(fname)
    print("\nFigure saved as '%s'" % fname)
...
for i in range(1, 18):
    ...
    ax.hist(graph_3[:, i - 1])
    ax.set(ylabel='Frequency', xlabel='Samplings from the prior distribution')
    ax.set_title("Histogram for state %s in 3.daphne from Graph-based Sampling" %i)
...
for i in range(1, 3):
    ...
    ax.hist(graph_4[:, i - 1])
    ax.set(ylabel='Frequency', xlabel='Samplings from the prior distribution')
    ax.set_title("Histogram for %s_0 in 4.daphne from Graph-based Sampling" % name[i - 1])
...
for i in range(3, 5):
    ...
    ax.hist(graph_4[:, i - 1])
    ax.set(ylabel='Frequency', xlabel='Samplings from the prior distribution')
    ax.set_title("Histogram for %s_1 in 4.daphne from Graph-based Sampling" %name[i - 1])
    fname = os.path.join("figs", "graph_4_%s" %i)
    plt.savefig(fname)
    print("\nFigure saved as '%s'" % fname)