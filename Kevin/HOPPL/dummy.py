...
def evaluate(exp, env=None, sig=None):  # TODO: add sigma, or something
    ...
    if op == 'if':  # conditional
        ...
        (parms, body) = args
        return Procedure(parms=parms[1:], body=body, env=env,
                         sig=sig), sig  # the first element will be the address which we can ignore?
    else:  # procedure call
        proc, sig = evaluate(op, env, sig)
        vals = [evaluate(arg, env, sig)[0] for arg in
                args[1:]]  # NOTE: I think we can skip the address here b/c we'll catch
